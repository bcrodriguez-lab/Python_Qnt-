"""
Wolkvox — Detener campaña.

Registro en /config-apis:
  - archivo: PararCampana
  - metodo: put
...
Invocación (payload):
  ...
  - campaign_id: identificador de la campaña a detener (obligatorio)
...
def put(api_config: dict, payload: dict | None = None) -> dict:
    """
    PUT hacia Wolkvox para detener una campaña.
    ...
    try:
        ...
        if response.ok:
            return {
                "success": True,
                "message": f"Campaña detenida correctamente (HTTP {response.status_code})",
        ...
        return {
            "success": False,
            "message": f"No se pudo detener la campaña (HTTP {response.status_code})",
            "status": response.status_code,
            "url": url,
            "request_headers": headers,
            "campaign_id": campaign_id,
        ...
        return {"success": False, "message": "Timeout al invocar Parar campaña Wolkvox."}
